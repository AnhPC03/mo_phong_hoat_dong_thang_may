...
public class Elevator {
	...
	public void setCoord() {
		...
		for (int i = 0; i < listHuman.size(); i++) {
			if (!listHuman.get(i).isInElevator() && listHuman.get(i).getCoordX() >= 500) { //dang di ra thang may
				humanOutElevator++;
			}
		}

		this.setFloor(
				(int) ((building.getHeight() - coordY) / (building.getHeight() / building.getNumberFloors())) + 1); //floor ma thang may dang o
		this.stop = false;
		for (int i = 0; i < listHuman.size(); i++) {
			if (listHuman.get(i).isInElevator()) {
				if (listHuman.get(i).getCoorYFinish() == coordY) {            //nguoi nao do den diem ket thuc thi dung thang may
					this.stop = true;
				}
			}
			if (!listHuman.get(i).isInElevator()) {
				if (listHuman.get(i).getCoordYStart() == coordY && listHuman.get(i).getCoordX() >= 500
					...
					if (humanInElevator > 0) {
						...
						if (nextHumanGoIn == null || nextHumanGoIn == listHuman.get(i)) { // khong co ai
							this.stop = true;
						}
					}
				}
		...
		if (this.stop == false) {    //thang may dang chuyen dong 
			if (humanInElevator > 0) {
				if (randomHumanInElevator.getCoorYFinish() < coordY) {   // xet ngau nhien mot nguoi de biet thang may dang di len hay xuong
					moveUp();
					this.isMovingUp = true;
			...
			} else if (humanOutElevator > 0) {         //khong con nguoi trong thang may, khach dang ra ngoai
				...
				if (nextHumanGoIn.getCoordYStart() < coordY) {    // thang may dang trong, nhung van con nguoi o ngoai va nguoi nay ơ tang khac
					moveUp();
					this.isMovingUp = true;
					this.isMovingDown = false;
				}
				...
				if (nextHumanGoIn.getCoordYStart() == coordY) {        // thang may dang trong, co nguoi dang o tang do dinh vao thang may  
					this.stop = true;
	...
	private Human humanMakeMinDistance() {
		...
		for (int i = 0; i < listHuman.size(); i++) {
			if (listHuman.get(i).getFloorStart() < listHuman.get(i).getFloorFinish()    //di len
					&& listHuman.get(i).getCoordYStart() == listHuman.get(i).getCoordY()   //van dang doi thang may
					&& listHuman.get(i).getCoordX() == 500) {                                //den vi tri cho thang may
				listHumanGoUp.add(listHuman.get(i));
			} else if (listHuman.get(i).getFloorStart() > listHuman.get(i).getFloorFinish()  //di xuong
					&& listHuman.get(i).getCoordYStart() == listHuman.get(i).getCoordY()      //dang doi thang may
					&& listHuman.get(i).getCoordX() == 500) {                                 //den vi tri cho thang may
		...
		if (listHumanGoUp.size() > 0) {
			...
			for (int i = 0; i < listHumanGoUp.size(); i++) {
				if (listHumanGoUp.get(i).getFloorStart() < minFloorStartGoUp) {
					...
					nextHumanGoUp = listHumanGoUp.get(i);                               //trong danh sach nguoi di len, di xuong tang thap nhat
				}
		...
		if (listHumanGoDown.size() > 0) {
			...
			for (int i = 0; i < listHumanGoDown.size(); i++) {
				if (listHumanGoDown.get(i).getFloorStart() > maxFloorStartGoDown) {
					...
					nextHumanGoDown = listHumanGoDown.get(i);                            //trong danh sach nguoi di xuong, di xuong tang cao nhat
				}
		...
		if (listHumanGoUp.size() > 0 && listHumanGoDown.size() == 0) {       // chi co nguoi di len
			humanMakeMinDistance = nextHumanGoUp;
		}
		if (listHumanGoDown.size() > 0 && listHumanGoUp.size() == 0) {           //chi co nguoi di xuong
			humanMakeMinDistance = nextHumanGoDown;
		}
		if (listHumanGoDown.size() > 0 && listHumanGoUp.size() > 0) {              // tinh toan xem di duong nao it ton kem nhat
